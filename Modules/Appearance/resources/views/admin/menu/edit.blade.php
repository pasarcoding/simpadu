...
@section('content')
    <div class="row">
        <div class="col-12">
            <form action="{{ route('admin.appearance.menu.update', $data->id) }}" method="POST" class="card">
                @csrf
                @method('PUT')
                <div class="card-body">
                    @if (session('error'))
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" id="name" class="form-control"
                                    placeholder="Masukkan Nama Menu" value="{{ old('name', $data->name) }}" required>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="page_origin" id="page_origin" class="form-control select2" required>
                                    @foreach (getPageOriginList() as $k => $v)
                                        <option value="{{ $k }}"
                                            {{ old('page_origin', $data->page_origin) == $k ? 'selected' : '' }}>
                                            {{ $v }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="appearance_page_id" id="appearance_page_id" class="form-control select2"
                                    ...
                                    @foreach ($pages as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('appearance_page_id', ($data->page_origin == 'in' && $data->appearance_page_id != null ? $data->appearance_page_id : $data->default_page_id)) == $item->id ? 'selected' : '' }}>
                                            {{ $item->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-lg-8 d-none">
                            <div class="form-group">
                                ...
                                <input type="url" name="url" id="url" class="form-control"
                                    placeholder="Masukkan Url" value="{{ old('url', $data->url) }}" required>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="behaviour_target" id="behaviour_target" class="form-control select2" required>
                                    ...
                                    @foreach (getAppearanceMenuBehaviourTargetList() as $k => $v)
                                        <option value="{{ $k }}"
                                            {{ old('behaviour_target', $data->behaviour_target) == $k ? 'selected' : '' }}>
                                            {{ $v }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="order" id="order" class="form-control"
                                    placeholder="Masukkan Urutan" value="{{ old('order', $data->order) }}" required>
                            </div>
                        </div>
                    </div>
                </div>
