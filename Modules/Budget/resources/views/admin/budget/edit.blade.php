...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item" aria-current="page">Edit Anggaran</li>
@endsection

@section('content')
    <div class="row">
        <div class="col-12">
            <form action="{{ route('admin.budget.update', $data->id) }}" method="POST" class="card">
                @csrf
                @method('PUT')
                <div class="card-body">
                    @if (session('error'))
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" id="title" class="form-control"
                                    placeholder="Masukkan Nama Anggaran" value="{{ old('title', $data->title) }}" required>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="bank_name" id="bank_name" class="form-control select2" required>
                                    ...
                                    @foreach (getBankList() as $k => $v)
                                        <option value="{{ $k }}"
                                            {{ old('bank_name', $data->bank_name) == $k ? 'selected' : '' }}>
                                            {{ $v }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="bank_number" id="bank_number" class="form-control"
                                    ...
                                    value="{{ old('bank_number', $data->bank_number) }}" required>
                            </div>
                        </div>
